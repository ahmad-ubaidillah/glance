#!/bin/bash
# =============================================================================
# Glance Full Installer - Everything including RAG + ML (~300MB+)
# =============================================================================
# Usage:
...
install_glance() {
    log_info "Installing Glance Full (this may take a while)..."
    if command -v uv &> /dev/null; then
        uv pip install -e ".[rag,local]" --quiet
        uv tool install -e ".[rag,local]" --quiet 2>/dev/null || true
    elif command -v pip3 &> /dev/null; then
        pip3 install -e ".[rag,local]" --quiet
    elif $PYTHON_CMD -m pip --version &> /dev/null; then
        $PYTHON_CMD -m pip install -e ".[rag,local]" --quiet
    else
        log_error "No pip found."; exit 1
    fi
    log_success "Glance Full installed!"
}

main() {
    echo -e "${BLUE}"
    echo "╔═══════════════════════════════════════════════════════════════════╗"
    echo "║              Glance Full Installer v1.0                           ║"
    echo "║         Everything: Core + TUI + RAG + ML (~300MB)                ║"
    echo "╚═══════════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
    ...
    echo ""
    echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║              Glance Full Installed!                               ║${NC}"
    echo -e "${GREEN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    echo "  ✅ RAG (semantic code search)"
    echo "  ✅ Local ML features"
    echo ""
    echo -e "${YELLOW}Start the dashboard:${NC}"
