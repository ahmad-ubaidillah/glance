...
def menu(stdscr, title, items, selected=0):
    ...
    while True:
        ...
        if len(items) > max_visible:
            scroll_info = f"↑↓ scroll ({scroll_offset + 1}-{scroll_offset + max_visible}/{len(items)})"
...
def show_page(stdscr, title, lines, scrollable=True):
    ...
    while True:
        ...
        if scrollable and total_lines > max_content_lines:
            footer_parts.append(f"↑↓ scroll ({scroll_offset + 1}-{scroll_offset + max_content_lines}/{total_lines})")
        ...
        if scrollable and total_lines > max_content_lines:
            if key in (curses.KEY_UP, ord("k")):
                ...
                scroll_offset = min(total_lines - max_content_lines, scroll_offset + max_content_lines)
            else:
                break
        else:
            break
...
def stats_screen(stdscr):
    try:
        ...
        if stats.issues_by_category:
            ...
            for cat, count in sorted(stats.issues_by_category.items(), key=lambda x: x[1], reverse=True)[:10]:
...
def execution_screen(stdscr):
    ...
    modes = [
        "sequential (1 LLM, 4 personas in order)",
        "parallel (3 LLMs at once)"
    ]
...
def run_review_screen(stdscr):
    show_page(stdscr, "Running Review", [
        "Starting review...",
        "",
